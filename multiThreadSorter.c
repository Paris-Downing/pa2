...
int formatcsv(char* inputfilepath, char* inputfile, char* inputColumn, char* outputDir){
	...
	*/
	...
	for(i = 0; i < cols; i++)
	{
		...
		for(j = 0; j < rows; j++)
		{
			strcpy(myPrinting[j][positions[i]], myStruct[j][i]);
	...
	for(i = 0; i < rows; i++)
		for(j = 0; j < cols; j++)
			if(myPrinting[i][j] == NULL)
				strcpy(myPrinting[i][j], "");
...
int csvfilehandler(char* inputfilepath, char* inputfile, char* inputColumn, char* outputDir){
	...
	FILE* output = fopen(formatOutput(inputfile, inputColumn, outputDir), "a");
	if(output == NULL){
